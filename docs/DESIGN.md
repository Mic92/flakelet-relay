...
1. proto, auth (SANs, `sub` and claim principals, policy), agent with
   transient unit and journal follow, one relay, single-wave SSE,
   `*_up` and `jobs_total` metrics, VM happy path.
2. Job table, reattach, replay, `/v1/jobs`, coalescing, waves, progress
   and idle timeout, journal tail, `push` retry and failover,
   `push login`, SRV discovery, JWKS cache, jti.
3. NixOS modules and flakelets. Relays on eve and eva, SRV under
   thalheim.io. step-ca ACME host certs and agents on eve, eliza and
   jamie. Move tribuchet and nixbot effects to `flakelet-push` and
   remove the ssh `flakelet-deploy` module.
4. Dashboard with OIDC login. Remaining metrics, alerts, GitHub Actions
   issuer.
